...
public class Controlador implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals("historial")) {
			...
			m.getPa(); 
			Paciente p1 = m.buscar(vb.getVr().getIdentificacion_().getText());
			p1.setSumaNC(m.getCu().sumatoriaNeurocognitivo(Integer.parseInt(vb.getVnc().getValores_evaluoDeficitAprendizaje().getSelectedItem().toString()), Integer.parseInt(vb.getVnc().getValores_evaluoDeficitAtencion().getSelectedItem().toString()), Integer.parseInt(vb.getVnc().getValores_evaluoImperactividad().getSelectedItem().toString())));
			 vb.getVnc().setVisible(false);
			...
			m.getPa(); 
			Paciente p1 = m.buscar(vb.getVr().getIdentificacion_().getText());
			p1.setSumaND(m.getCu().sumatoriaNeurocognitivo(Integer.parseInt(vb.getVnc().getValores_evaluoDeficitAprendizaje().getSelectedItem().toString()), Integer.parseInt(vb.getVnc().getValores_evaluoDeficitAtencion().getSelectedItem().toString()), Integer.parseInt(vb.getVnc().getValores_evaluoImperactividad().getSelectedItem().toString())));
            vb.getVnd().setVisible(false);
            p1.setCuidados(m.getCu().clasificarMatriz(p1.getClasificacion(), p1.getSumaNC(), p1.getSumaND()));
			JOptionPane.showMessageDialog(null, p1.getCuidados());
		}
